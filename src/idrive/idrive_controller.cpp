...
IDriveController::IDriveController(CanBus& can, UsbHidDevice& hid,
                                   const Config& config)
...
void IDriveController::Init() {
    ...
    auto joystick = std::make_unique<JoystickHandler>(
        hid_, config_.joystick_as_mouse, config_.joystick_move_step);
    ...
    auto touchpad = std::make_unique<TouchpadHandler>(
        hid_, config_.min_mouse_travel,
    ...
    can_.SetCallback([this](const CanMessage& msg) {
        OnCanMessage(msg);
    });
...
void IDriveController::HandleTouchpadMessage(const CanMessage& msg) {
    ...
    if (touchpad_init_ignore_counter_ < config::kTouchpadInitIgnoreCount &&
        ...
        ESP_LOGI(kTag, "Touchpad ignoring message %d/%d",
                 touchpad_init_ignore_counter_, config::kTouchpadInitIgnoreCount);
        return;
    }
